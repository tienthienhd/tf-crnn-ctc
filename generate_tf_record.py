...
def main():
    df_label = pd.read_csv(config.dataset['label_file'], header=0, dtype={'filename': str, 'label': str})
    ...
    output_path = os.path.join(config.dataset['data_dir'], f'{config.dataset["data_name"]}_train.records')
    create_tf_record(config.dataset['image_dir'], df_label_train, config.dataset['charset'], output_path,
                     des='Train dataset')

    output_path = os.path.join(config.dataset['data_dir'], f'{config.dataset["data_name"]}_test.records')
    create_tf_record(config.dataset['image_dir'], df_label_test, config.dataset['charset'], output_path,
                     des='Test dataset')
